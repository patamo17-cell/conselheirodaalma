...
const categoriasConfig: Record<string, [string, string][]> = {
...
const criarExplicacao = (categoria: string) => {
    return `Meu amado, receba essa palavra profética diretamente no seu espírito! Eu sei que a luta contra essa ${categoria.toLowerCase()} tem sido intensa, mas declare comigo agora: 'O meu Deus é maior e a vitória já é minha!'. Essa situação não tem poder sobre a sua vida porque você está debaixo da unção e da proteção do Altíssimo. Em nome de Jesus, eu profetizo que as correntes da opressão estão caindo por terra hoje pelo poder do Espírito Santo. O Senhor está soprando um fôlego novo sobre você, trazendo paz, provisão e abrindo portas onde o mundo disse que estava fechado. Não aceite o desânimo, pois o Senhor já assinou o seu decreto de libertação e prosperidade. Tome posse da sua benção, declare a sua vitória e descanse no poder do sangue de Jesus, pois o melhor de Deus ainda está por vir na sua vida!`;
};
...
const banco = { versiculos: [] as any[] };
let idCont = 1;

for (const [cat, versiculos] of Object.entries(categoriasConfig)) {
    for (const [ref, texto] of versiculos) {
        const item = {
            id: idCont,
            codigo: `V${idCont.toString().padStart(3, '0')}`,
            categoria: cat,
            sentimentos: sentimentosMap[cat],
            versiculo: texto,
            referencia: ref,
            explicacao: criarExplicacao(cat)
        };
        banco.versiculos.push(item);
        idCont++;
    }
}

fs.writeFileSync('banco_versiculos.json', JSON.stringify(banco, null, 2), 'utf-8');
console.log('Banco de versículos gerado com sucesso!');
